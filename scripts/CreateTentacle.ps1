...
for ($tentacleNo = 1; $tentacleNo -le $numTentacles; $tentacleNo++)
{
    ...
    #Write-Output "C:\Program Files\Octopus Deploy\Tentacle\Tentacle.exe" register-with --instance "$name" --name "$name" --publicHostName "https://cd:$port" --server "$octopusUrl" --apiKey="$apiKey" --role "$role" --environment "$environment" --comms-style TentaclePassive --console  --policy="The Cattle Yard"
    #return
    ...
    & "C:\Program Files\Octopus Deploy\Tentacle\Tentacle.exe" register-with --instance "$name" --name "$name" --publicHostName "cd" --server "$octopusUrl" --apiKey="$apiKey" --role "$role" --environment "$environment" --comms-style TentaclePassive --console  --policy="The Cattle Yard"
